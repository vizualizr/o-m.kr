...
**Project Overview**: This project is an independent data journalism site. It provides insight on current issues via interactive data storytelling technology. The site delivers content on the **Astro framework**, utilizing **d3.js** and other **client-side rich content web-technology**.
...
For a detailed guide, refer to `development_guide.md`.

### Data Visualization
- **Library**: **D3.js (v7)** loaded via CDN.
- **Patterns**: Standard D3 selection, linear scaling, and axis generation.
- **Components**: `Scroller.astro` (scrollytelling), `graph.html` (standalone).

### Design System
- **Theme**: "Paper-like" aesthetic with custom `oklch` color scale (`--color-paper-*`) in `theme.css`.
- **Typography**: `font-serif` (Georgia) for reading, `font-sans` (System UI) for UI.
- **Styling**: **Tailwind CSS (v4)** utility classes and **FlyonUI** components.

### Deployment
- **Build**: `npm run build` (Astro SSG).
- **Environment**: Node.js required; local HTTPS via Vite plugin.
