...
- Fix syntax error: Change `module.exports` to `export default`. Since the project is `"type": "module"`, `.mjs` files must use ESM syntax. Although Tailwind v4 prefers CSS variables, it still reads this config if present.

### CSS Structure Standardization

#### [MODIFY] [theme.css](file:///d:/yonggeun/porter/git/o-m.kr/production/src/styles/theme.css)
- Remove `@import "tailwindcss";` from the top of this file **if** it is primarily intended to be imported into `global.css`. 
- > [!NOTE]
...
- Add `@import "tailwindcss";` to the top of this file. This file is imported directly in `SwissLayout.astro`, making it a CSS entry point. Without the import, the `@theme` block inside is unrecognized.

#### [MODIFY] [global.css](file:///d:/yonggeun/porter/git/o-m.kr/production/src/styles/global.css)
- **Move `@import "tailwindcss"` to the top.** Standard CSS requires all `@import` statements to appear before any other rules (like `@font-face` or style rules). In `global.css`, it currently appears after several `@font-face` declarations, which is invalid CSS and can break the Tailwind v4 parser.
- **Deduplicate imports.** Remove the redundant `@import "tailwindcss"` from `theme.css` if it's already provided by the parent `global.css`.
...
### Manual Verification
- Open `src/styles/theme.css` (and others) in the IDE and verify that `@theme` is correctly recognized (or at least not flagged as an error).
